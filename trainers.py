...
def I2T(
    ...
    for i, data in enumerate(dataloader):
        ...
        loss = criterion_MSE(features, caps)
        # cap_features = textmodel(caps)
        # # print(cap_features.shape,features.shape)
        # features_cross = torch.cat([features.unsqueeze(1), cap_features.unsqueeze(1)], dim=1)
        # loss_cross = criterion(features_cross,torch.from_numpy(label))
        loss_sum += loss.item()
