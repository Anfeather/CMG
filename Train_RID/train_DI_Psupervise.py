...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch, predictW, predictW_optimizer):
    ...
    gmm = GMM(n_components=4).fit(caps)
    center = gmm.means_
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # Flatten image
        encoder_out = imgs.view(batch_size, -1, encoder_dim) 


        _, f2 = torch.split(features, [batch_size, batch_size], dim=0)
        ...
        k_t = 0
        for data_w, data_w2, data_t in zip(encoder_out, f2, target_weights):
            ...
            k_t += 1
        loss_CL = criterion(distilled_information, distilled_information2)
