...
class non_iid_MSCOCODataset_Distillation_flower(Dataset):
...
def Flower_image_with_CMA_D( mode,   data_dir, user_dict, batch_size, normalize=True, norm_layer=None, size=256, data_name="coco_5_cap_per_img_5_min_word_freq", F="N",FineGrain=0):
    ...
    if F == "OOD":
        trainset = non_iid_MSCOCODataset_Distillation_flower(data_dir, data_name,'train', user_dict, 1, transform_train, FineGrain)
        ...
        trainset = non_iid_MSCOCODataset_Distillation_flower(data_dir, data_name,'train', user_dict, 0, transform_train,FineGrain)
        testset = non_iid_MSCOCODataset_Distillation_flower(data_dir, data_name, 'test', user_dict, 0, transform_test,FineGrain)
...
def wiki_image( mode,   data_dir, user_dict, batch_size, normalize=True, norm_layer=None, size=256, data_name="coco_5_cap_per_img_5_min_word_freq", F="N",FineGrain=0):


    transform_test = [ transforms.Resize((80,80)), transforms.ToTensor()]
    transform_train = [ transforms.Resize((80,80)), transforms.ToTensor()]
    ...
    if F == "OOD":
        trainset = non_iid_MSCOCODataset_Distillation_wiki(data_dir, data_name,'train', user_dict, 1, transform_train, FineGrain)
        ...
        trainset = non_iid_MSCOCODataset_Distillation_wiki(data_dir, data_name,'train', user_dict, 0, transform_train,FineGrain)
        testset = non_iid_MSCOCODataset_Distillation_wiki(data_dir, data_name, 'test', user_dict, 0, transform_test,FineGrain)
...
class non_iid_MSCOCODataset_Distillation_wiki(Dataset):
    ...
    def __init__(self, data_folder, data_name, flag, user_dict, user_id, transform, FineGrain):
        ...
        if flag == "train":
            for i in data_distribution:
                ...
                # print("**********************")
                # print(os.path.join(data_train_path,  'image_processed.hdf5'))
                self.h = h5py.File(os.path.join(data_train_path,  'image_processed.hdf5'), 'r')
                self.probs.extend(self.h['prob'])
                self.imgs0.extend(self.h['images0'])
                self.imgs1.extend(self.h['images1'])
                self.imgs2.extend(self.h['images2'])
                self.imgs3.extend(self.h['images3'])
                self.imgs4.extend(self.h['images4'])
                self.imgs5.extend(self.h['images5'])
                self.imgs6.extend(self.h['images6'])
                self.imgs7.extend(self.h['images7'])
                self.imgs8.extend(self.h['images8'])
        elif flag == "test":
            for i in data_distribution:
                data_train_path = data_folder + str(i)
        ...
        self.dataset_size = len(self.imgs0)
    def __getitem__(self, i):
        # print(len(self.imgs0),len(self.imgs8))
        img = [self.imgs0[i], self.imgs1[i], self.imgs2[i], self.imgs3[i], self.imgs4[i], self.imgs5[i], self.imgs6[i], self.imgs7[i], self.imgs8[i]]
        probs = torch.FloatTensor(self.probs[i]) 
        return img,  probs
